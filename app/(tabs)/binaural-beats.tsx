...
export default function BinauralBeats() {
  // Track premium status (for demo, default to false)
  ...
  const toggleLeftSelection = (idx: number) => {
    // Only allow 1 premium loop per ear for free users
    if (!isPremium && LOOPS[idx].isPremium) {
      handlePremiumPress();
      return;
    }
    setLeftSelections(prev => {
  ...
  const getSelectedLoops = (selections: number[]) => {
    ...
    // If not premium and any selected loop is premium, show premium modal
    if (!isPremium && (
      leftSelections.some(idx => LOOPS[idx].isPremium) || rightSelections.some(idx => LOOPS[idx].isPremium)
    )) {
      handlePremiumPress();
      return;
    }
    ...
    >      <View style={styles.header}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.row}>
          {LOOPS.map((loop, idx) => {
            const locked = loop.isPremium && !isPremium;
            return (
              <TouchableOpacity
                key={loop.name + 'L'}
                style={[
                  ...
                  locked && { opacity: 0.5, borderWidth: 1, borderColor: '#aaa' }
                ]}
                onPress={() => locked ? handlePremiumPress() : toggleLeftSelection(idx)}
                disabled={locked}
              >
                <Text style={[
                  styles.loopBtnText,
          ...
          {LOOPS.map((loop, idx) => {
            ...
            return (
              <TouchableOpacity
                ...
                onPress={() => locked ? handlePremiumPress() : toggleRightSelection(idx)}
                disabled={locked}
              >
                <Text style={[
                  styles.loopBtnText,
        ...
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'center', alignItems: 'center' }}>
          <View style={{ backgroundColor: '#fff', borderRadius: 16, padding: 32, alignItems: 'center', maxWidth: 320 }}>
            ...
            <Text style={{ fontSize: 16, color: '#333', marginBottom: 20, textAlign: 'center' }}>
              Unlock all binaural beats and more features!
            </Text>
            <TouchableOpacity
